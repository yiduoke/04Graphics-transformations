...
struct matrix * make_scale(double x, double y, double z) {
  struct matrix* dilate = new_matrix(4, 4);
  dilate->lastcol = dilate->cols;
...
float deg_to_rad(double degree){
...
struct matrix * make_rotX(double theta) {
  ...
  rotate->lastcol = rotate->cols;

  ident(rotate);

  rotate->m[1][1] = cos(theta);
  rotate->m[1][2] = -1 * sin(theta);
